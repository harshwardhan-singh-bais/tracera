...
from tracera.mcp.server import MCP_API_VERSION, MCP_MIN_CLIENT_VERSION, TraceraMCPServer
...
def test_authentication_with_api_key():
    ...
    settings = Settings(
        tracera_mcp_api_key="test-secret-key-123", tracera_mcp_enforce_workspace_boundaries=True
    )
    server = TraceraMCPServer(settings=settings, workspace_path=Path("."))
