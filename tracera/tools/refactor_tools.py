...
class PlanRefactoringTool(Tool):
    ...
    async def execute(self, operation: str, symbol: str, target: str = "") -> ToolResult:
        ...
        if operation == "rename":
            ...
            lines.append(
                f"**Symbol defined in:** `{info.get('file_path', '?')}` L{info.get('start_line', '?')}-{info.get('end_line', '?')}\n"
            )
            ...
            for cid in callers[:20]:
                ...
                if n:
                    lines.append(
                        f"- `{n.get('file_path', '?')}`:{n.get('start_line', '?')} — `{n.get('name', '?')}`"
                    )
            ...
            if callers:
                ...
                lines.append(
                    f"Replace `{symbol}` with `{new_name}` in all import statements across the files above."
                )
            ...
            lines.append("3. Update imports in all referencing files")
            callers = graph.get_callers(node_id)
            if callers:
            ...
            lines.append(
                f"**Extract:** Create new symbol `{target or 'extracted_func'}` from `{symbol}`\n"
            )
            lines.append("### Steps:")
            ...
            lines.append(
                f"3. Replace the original block with a call to `{target or 'extracted_func'}`"
            )
            lines.append("4. Update imports if the new symbol is in a different module")
    ...
    async def execute(self, symbol: str) -> ToolResult:
        ...
        if caller_count > 0:
            ...
            for cid in callers[:10]:
                ...
                if n:
                    lines.append(
                        f"- `{n.get('name', '?')}` in `{n.get('file_path', '?')}`:{n.get('start_line', '?')}"
                    )
...
class CheckDeleteSafeTool(Tool):
    ...
    async def execute(self, symbol: str) -> ToolResult:
        ...
        if not node_id:
            return ToolResult.ok(
                self.name, "", f"Symbol '{symbol}' not found — may already be deleted."
            )
...
class GetPrRiskProfileTool(Tool):
    ...
    _params = {
        ...
        "properties": {
            "ref": {
                "type": "string",
                "description": (
                    "Git ref or branch name: 'HEAD' for uncommitted, branch name for a branch diff."
                ),
                "default": "HEAD",
            },
        },
    ...
    async def execute(self, ref: str = "HEAD") -> ToolResult:
        ...
        config_files = sum(
            1 for f in changed_files if any(k in f.lower() for k in ["config", ".env", "settings"])
        )
