...
file cut mean context bytes by 31% for hybrid (8806 → 6088), 43% for BM25
(10250 → 5841) and 35% for dense (8145 → 5287) — *at unchanged* ``k``. A
file's later chunks are its larger bodies; only the top-ranked one survives, and
the window refills from files that would otherwise have been cut off.
...
of an already-kept file. Cap 2 measured as the free point — hybrid recall@5 moved
0.9458 → 0.9417 (half a query) with recall@10 flat, for −12% bytes — and is
the default the agent tool uses. Cap 1 is the aggressive setting.
...
in the top ``k``: hybrid's baseline precision@5 of 0.62 was largely that
redundancy. Compare recall, MRR and nDCG across arms — not precision.
"""
